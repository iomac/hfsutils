 ...
 */
static int fillchain(hfsvol *vol, bucket **bptr, size_t *count)
{
 ...
 */
static
int flushchain(hfsvol *vol, bucket **bptr, size_t *count)
{
    bucket *blist[HFS_BLOCKBUFSZ], **start = bptr;
 ...
 */
static int dobuckets(hfsvol *vol, bucket **chain, size_t len,
              int (*func)(hfsvol *, bucket **, size_t *))
{
    size_t count, i;
    int result = 0;
